...
def process_files(crossfile, parametricData, pakageAndPinout, recipe):
    """Step 1: Validate input and compare PLs"""
...
def load_all_excel_sheets(file_list):
    """Load all sheets from all Excel files"""
    cache = {}
...
def get_single_row_all_files(file_cache, part_value, company_value, PackaePinout_val):
    """Merge one row from multiple Excel files"""
...
def merge_from_crossesparts(crosses_file, target_files):
    """Step 2: Merge data from crosses file"""
...
def validate_core_upgrade_equal(newcross_file, finalmerged_file, lookup_file1, lookup_file2, loadLookUp):
    """Step 3: Validate core, upgrade, and equal features"""
...
def main():
    ...
    if st.session_state.processed:
        ...
        total_records = len(organized_df)
        cross_matches = len(organized_df[organized_df['Status'] == 'Cross'])
        not_cross = len(organized_df[organized_df['Status'] == 'Not Cross'])
        different_pls = len(organized_df[organized_df['Status'] == 'Different PLs'])
        not_found = len(organized_df[organized_df['Status'] == 'Not Found Data'])
        
        # Grade breakdown
        grade_counts = organized_df['Grade'].value_counts()
        drop_in_a = grade_counts.get('Drop-in A', 0)
        ...
        with tab1:
            st.dataframe(organized_df.head(100), use_container_width=True)
        
        with tab2:
            st.dataframe(st.session_state.results['validation'].head(100), use_container_width=True)
        
        with tab3:
            st.dataframe(st.session_state.results['merged'].head(100), use_container_width=True)
        
        with tab4:
            st.dataframe(st.session_state.results['step1'].head(100), use_container_width=True)
        ...
        with col1:
            organized_excel = save_to_excel(organized_df)
            st.download_button(
        ...
        with col2:
            validation_excel = save_to_excel(st.session_state.results['validation'])
            st.download_button(
        ...
        with col3:
            merged_excel = save_to_excel(st.session_state.results['merged'])
            st.download_button(
